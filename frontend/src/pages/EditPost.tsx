...
import { ArrowLeft, Save, Eye, Trash2, X, Bold, Italic, List, ListOrdered } from "lucide-react";
...
export default function EditPost({}: Props) {
  ...
  useEffect(() => {
    ...
    loadPost();
  }, [postSlug, navigate, reset, editor]);
  ...
  const onSubmit = async (data: EditPostForm) => {
    ...
    try {
      const postData: UpdateBlogPostRequest = {
        title: data.title,
        content: content,
      ...
      const response = await brain.update_blog_post({ postId }, postData);
      const result = await response.json();
      
      toast.success(data.is_published ? "Post updated and published!" : "Draft updated successfully!");
  ...
  const deletePost = async () => {
    ...
    try {
      await brain.delete_blog_post({ postId });
